...
export default function HomeScreen({ navigation }) {
  ...
  const fiveFingerGesture = Gesture.Manual()
    .onTouchesDown((event, state) => {
      if (event.numberOfTouches === 5) {
        ...
        }, 2000); // 2000 ms (2 Sekunden) drücken notwendig
      }
    })
    .onTouchesUp(() => {
  ...
  useEffect(() => {
    const loadRatings = async () => {
      // This function is not needed as ratings are already loaded in the context
    };
  ...
  const resetRatings = async () => {
    ...
    await resetProgress();
    //Alert.alert('Erfolg', 'Die Bewertungen wurden zurückgesetzt.');
  };
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <GestureDetector gesture={fiveFingerGesture}>
        <View style={styles.container}>
          <Text style={styles.greetingText}>Wilkommen!</Text>
          <Text style={styles.greetingText}>Wähle ein Quiz aus mit dem du Starten möchtest.</Text>
