...
export default function QuizScreen({ route, navigation }) {
  ...
  const checkAnswer = () => {
    ...
    if (currentQuestion.type === 'order') {
      if (JSON.stringify(selectedOptions) === JSON.stringify(currentQuestion.correctAnswer)) {
        ...
        resetIncorrectOrderOptions();  // Nur die falschen Optionen zurücksetzen
      }
      ...
      if (JSON.stringify(selectedValues) === JSON.stringify(currentQuestion.correctAnswer)) {
        ...
        setShowCorrectImage(false);
        setStars(Math.max(0, stars - 1));
        resetOptions();  // Hier sicherstellen, dass alle Optionen zurückgesetzt werden
      }
  ...
  const nextQuestion = async () => {
    if (!selectedOptions.some(option => option !== null) && comparisonAnswer === null) {
      // Wenn keine Option ausgewählt ist, setze die Sterne für diese Frage auf 0
      setStars(0);
    }else{
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.quizContainer}>
        ...
        />
        {showCorrectImage && <Image source={require('../assets/Eule_Super.png')} style={styles.correctImage} />}
        {result && (
  <View
    style={[
